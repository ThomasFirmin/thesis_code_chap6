...
class Objective(object):
    def __init__(
        ...
        for key in ["all", "vote", "2gram", "3gram", "log", "svm"]:
            # for key in ["all", "vote", "log", "svm"]:
    ...
    def test(self, network):
        ...
        for step, batch in enumerate(dataloader):
            ...
            for layer in self.recorders:
                ...
                self.spike_record[layer][0] = (
                    ...
                    .get("s")
                    .squeeze()
                    .reshape(int(self.time), *network.layers[layer].shape)
                )
        ...
        for key in all_predictions:
            self.accuracy_test[key] /= self.n_valid
    ...
    def accuracy(self):
        if self.errored:
            ...
            d = {
                "valid": self.accuracy_test[self.decoder],
                "train": self.accuracy_train_epoch[self.decoder][self.n_epochs - 1],
                "train_time": self.train_time,
                "test_time": self.test_time,
                "total_time": self.total_time,
        ...
        for key in ["all", "vote", "2gram", "3gram", "log", "svm"]:
            # for key in ["all", "vote", "log", "svm"]:
            d[f"{key}_valid"] = self.accuracy_test[key]
    ...
    def __call__(self, *args, load=False, train=True, **kwargs):
        ...
        try:
            for key, c in network.connections.items():
                ...
                if not isinstance(c.update_rule, NoOp):
                    ...
                    train_zeronparameters += (c.w == 1.0).sum()
                    train_onenparameters += (c.w == 0.0).sum()
                    train_95nparameters += (c.w > 0.95).sum()
                    train_05nparameters += (c.w < 0.05).sum()
        except Exception as e:
            print(traceback.format_exc(), file=sys.stderr)
            print(e, file=sys.stderr)
            error = 3
        ...
        results["trainable_zeros"] = train_zeronparameters.item()
        results["trainable_ones"] = train_onenparameters.item()
        results["trainable_95"] = train_95nparameters.item()
        results["trainable_05"] = train_05nparameters.item()
        results["error"] = error
