...
class LocalReservoir(AbstractNetwork):
    ...
    def __init__(
        ...
        # Local
        conv_out, name = self._local(0, input_layer, "inpt")

        # Input to Reservoir
        # res_layer, name = self._reservoir(0, conv_out, name)

        output_layer, name = self.outpt_layer(conv_out, name, self.n_out)
    ...
    def outpt_layer(self, inputs, in_name, nneurons):
        ...
        if self.lateral_inhibition:
            rrange = torch.arange(nneurons).reshape(1, nneurons, 1)
