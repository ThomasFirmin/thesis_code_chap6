...
def get_mnist_rate_lava_cnt_extended_rng():
    values = ArrayVar()

    values.append(FloatVar("train_size", 0.1, 1.0))

    values.append(IntVar("frames", 10, 50))

    values.append(IntVar("epochs", 1, 20))

    values.append(IntVar("batch_size", 30, 300))

    values.append(FloatVar("rate_true", 0.1, 0.9))
    values.append(FloatVar("rate_false", 0.01, 0.09))

    # Architecture
    values.append(IntVar("c1_filters", 1, 48))
    values.append(IntVar("c2_filters", 1, 48))

    values.append(IntVar("c1_k", 4, 12))
    values.append(IntVar("c2_k", 4, 12))

    ## SLAYER
    values.append(FloatVar("learning_rate", 0.001, 0.1))
    values.append(FloatVar("mu", 0.8, 0.999))
    values.append(FloatVar("gamma", 0.9, 1.0))

    values.append(FloatVar(f"tau_grad", 0.005, 1))
    values.append(FloatVar(f"scale_grad", 0.1, 1))
    ## Neurons params
    for layer in ["c1", "c2", "a1", "a2", "o"]:
        values.append(FloatVar(f"threshold_{layer}", 0.5, 8))

        values.append(FloatVar(f"threshold_step_{layer}", 0.05, 0.40))

        values.append(FloatVar(f"current_decay_{layer}", 0.05, 0.5))
        values.append(FloatVar(f"voltage_decay_{layer}", 0.01, 0.2))
        values.append(FloatVar(f"threshold_decay_{layer}", 0.01, 0.5))
        values.append(FloatVar(f"refractory_decay_{layer}", 0.05, 0.5))

        if layer not in ["a1", "a2", "o"]:
            values.append(FloatVar(f"dropout_{layer}", 0.01, 0.90))
    return values
...
def get_shd_lava_cnt_ext_rng():
    values = ArrayVar()

    values.append(FloatVar("train_size", 0.2, 1.0))

    values.append(IntVar("frames", 200, 400))

    values.append(IntVar("epochs", 1, 35))

    values.append(IntVar("batch_size", 20, 300))

    values.append(FloatVar("rate_true", 0.1, 0.9))
    values.append(FloatVar("rate_false", 0.01, 0.09))

    # Architecture
    values.append(IntVar("f1_n", 100, 1000))
    values.append(IntVar("f2_n", 100, 1000))
    values.append(IntVar("f3_n", 100, 1000))
    ## SLAYER
    values.append(FloatVar("learning_rate", 0.001, 0.1))
    values.append(FloatVar("beta_1", 0.8, 1.0))
    values.append(FloatVar("beta_2", 0.8, 1.0))
    ...
    for layer in ["f1", "f2", "f3", "o"]:
        ...
        values.append(FloatVar(f"threshold_{layer}", 0.5, 10))

        values.append(FloatVar(f"threshold_step_{layer}", 0.05, 0.5))
...
def old_get_mnist_lava_cnt(convert=True):
    if convert:
        values = ArrayVar(converter=ArrayDefaultC())
    else:
        values = ArrayVar()
...
def old_get_dvs_lava_cnt(convert=True):
    if convert:
        values = ArrayVar(converter=ArrayDefaultC())
    else:
        values = ArrayVar()
...
def old_get_mnist_spikingjelly_cnt(convert=True):
    if convert:
        values = ArrayVar(converter=ArrayDefaultC())
    else:
        values = ArrayVar()
    ...
    ## Neurons params
    values.append(FloatVar("threshold", 0.1, 4, converter=FloatMinMax()))
...
def old_get_dvs_spikingjelly_cnt(convert=True):
    if convert:
        values = ArrayVar(converter=ArrayDefaultC())
    else:
        values = ArrayVar()
...
def get_mnist_rate_spikingjelly_cnt(convert=True):
    if convert:
        values = ArrayVar(converter=ArrayDefaultC())
    else:
        values = ArrayVar()
    ...
    values.append(FloatVar("alpha", 2, 10, converter=FloatMinMax()))

    return values


def get_mnist_rate_spikingjelly_cnt_extended(convert=True):
    if convert:
        values = ArrayVar(converter=ArrayDefaultC())
    else:
        values = ArrayVar()
    ...
    for layer in ["f1", "f2", "o"]:
        ...
        values.append(FloatVar(f"alpha_{layer}", 2, 10, converter=FloatMinMax()))
...
def get_mnist_rate_spikingjelly_cnt_extended_rng():
    values = ArrayVar()
    ...
    values.append(IntVar("batch_size", 20, 200))
    ...
    ## Neurons params
    for layer in ["f1", "f2", "o"]:
        values.append(FloatVar(f"threshold_{layer}", 0.1, 4))

        values.append(FloatVar(f"threshold_decay_{layer}", 1, 40))

        values.append(FloatVar(f"alpha_{layer}", 2, 10))

    values.append(FloatVar("dropout", 0.01, 0.90))

    return values


def get_shd_spikingjelly_cnt(convert=True):
    if convert:
        values = ArrayVar(converter=ArrayDefaultC())
    else:
        values = ArrayVar()
    ...
    values.append(
        FloatVar("train_size", 0.15, 1.0, converter=FloatMinMax(), sampler=loguniform)
    )
    ...
    values.append(FloatVar(f"threshold", 0.1, 2, converter=FloatMinMax()))
    values.append(FloatVar(f"threshold_decay", 1, 40, converter=FloatMinMax()))
...
def get_shd_spikingjelly_cnt_ext(convert=True):
    if convert:
        values = ArrayVar(converter=ArrayDefaultC())
    else:
        values = ArrayVar()
    ...
    # Architecture
    values.append(
        IntVar("f1_n", 50, 1000, converter=IntMinMax(), sampler=disc_loguniform)
    )
    values.append(
        IntVar("f2_n", 50, 1000, converter=IntMinMax(), sampler=disc_loguniform)
    )
    values.append(
        IntVar("f3_n", 50, 1000, converter=IntMinMax(), sampler=disc_loguniform)
    )
...
def get_shd_spikingjelly_cnt_ext_rng():
    values = ArrayVar()

    values.append(FloatVar("train_size", 0.15, 1.0))
    ...
    # Architecture
    values.append(IntVar("f1_n", 50, 1000))
    values.append(IntVar("f2_n", 50, 1000))
    values.append(IntVar("f3_n", 50, 1000))

    for layer in ["f1", "f2", "f3", "o"]:
        ## Neurons params

        values.append(FloatVar(f"threshold_{layer}", 0.1, 2))
        values.append(FloatVar(f"threshold_decay_{layer}", 1, 40))
