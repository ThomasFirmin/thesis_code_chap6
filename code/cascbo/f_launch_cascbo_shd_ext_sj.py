...
from geleeepicee.optim_conv import FfSHDExtended
from geleeepicee.objective import Objective
from geleeepicee.early_stoping import NonSpikingPercentageStopping
from load_dataset import DataGetter
from search_spaces import get_shd_spikingjelly_cnt_ext
...
from zellij.strategies.continuous import CASCBO
...
parser.add_argument("--save", type=str, default="test_casbo_shd_ext_sj")
...
model = Objective(
    ...
    beta_test=0.2,
    gpu=gpu,
    early_stopping=NonSpikingPercentageStopping("outpt", 1, 0.2),
    max_epochs=35,
    optimizer="ADAM",
)

loss = Loss(
    objective=[Maximizer("test"), DoNothing("approximate_cost")],
...
# Decision variables
values = get_shd_spikingjelly_cnt_ext()
